...
class MyGame(arcade.Window):
    ...
    def on_draw(self):
        ...
        # Render the text
        arcade.draw_text(f"Your Score: {self.score}", 300, 580, arcade.color.WHITE, 16)

        # Draw game over if the game state is such
        if self.game_state == GAME_OVER:
            arcade.draw_text(f"NOOB SHIT :)", 20, 300, arcade.color.WHITE, 40)
    ...
    def on_mouse_press(self, x, y, button, modifiers):
        ...
        if len(self.player_bullet_list) < MAX_PLAYER_BULLETS:
            ...
            # Create a bullet
            bullet = arcade.Sprite(":resources:images/space_shooter/meteorGrey_small2.png", SPRITE_SCALING_LASER)
